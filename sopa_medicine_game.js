// sopa_medicine_game.js - Lógica para el juego de Sopa de Letras (Tema Medicina)
...
// Palabras principales a encontrar (15 palabras) para MEDICINA
const MAIN_WORDS = [
    "SALUD", "DOCTOR", "VIRUS", "FIEBRE", "DOLOR",
    "MEDICINA", "HOSPITAL", "ENFERMO", "VACUNA", "SINTOMA",
    "TERAPIA", "CIRUGIA", "DIAGNOSIS", "FARMACIA", "HIGIENE"
];

// Palabras sorpresa (no visibles en la lista, pero dan puntos extra) para MEDICINA
const SURPRISE_WORDS = [
    "ANATOMIA", "PATOLOGIA", "RADIOGRAFIA", "PEDIATRIA", "CARDIOLOGIA",
    "NEUROLOGIA", "ONCOLOGIA", "BACTERIA", "ANTIBIOTICO", "VITAMINA"
];

const ALL_WORDS = [...MAIN_WORDS, ...SURPRISE_WORDS];

let gameGrid = [];
let placedWords = []; // { word: "SALUD", cells: [{row:0, col:0}, ...], isFound: false, isBonus: false }
...
async function updatePlayerBalance(gold = 0, diamonds = 0) {
    ...
    if (sessionError) {
        ...
        Swal.fire({
            ...
            customClass: { popup: 'swal2-custom-game-over' } // Corregido: 'swal2-custom-game-over'
        });
        return;
    }
